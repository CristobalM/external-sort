...
parsed_options parse_cmline(int argc, char **argv){
  ...
  if(!has_tmp_dir){
    ...
    auto fname_template = (std::filesystem::path(tmp_base) / std::filesystem::path("tmpsort_XXXXXX")).string();
    auto mut_fname_template = std::make_unique<char[]>(fname_template.size() + 1);
    std::copy(fname_template.begin(), fname_template.end(), mut_fname_template.get());
    mut_fname_template[fname_template.size()] = '\0';
    char *tmp_dir = mkdtemp(mut_fname_template.get());
    if(!tmp_dir) throw std::runtime_error("Couldn't generate tmp dir");
    out.tmp_dir = tmp_dir;
  }
